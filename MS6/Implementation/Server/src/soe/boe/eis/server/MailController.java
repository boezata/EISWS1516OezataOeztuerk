...
public class MailController {
	...
	public static void sendNewUserMail(UserObject newUserObject, String secCode) {
		...
		String sender = Settings.getInstance().getSendingMailAddress();
		String reciever = newUserObject.getEmail();
		String messageContent = "<a href=\"" + Settings.getInstance().getWebsiteLink() + "eis.php?mode=reg&mail="
				+ newUserObject.getEmail() + "&secCode=" + secCode + "\">Account aktivieren</a>";
		final String mailServerUserName = Settings.getInstance().getMailServerUserName();
		final String mailServerPassword = Settings.getInstance().getMailServerPassword();
	...
	public static void sendFergotPasswordMail(String mailAddress, String secCode) {
		...
		String messageContent = "<a href=\"" + Settings.getInstance().getWebsiteLink() + "eis.php?mode=fpw1&mail=" + mailAddress
	...
	private static void sendMail(final String mailServerUserName, final String mailServerPassword, String sender,
		...
		properties.setProperty("mail.host", Settings.getInstance().getMailServerAddress());
		properties.setProperty("mail.smtp.starttls.enable", "true");
		properties.put("mail.smtp.port", Settings.getInstance().getMailServerPort());
